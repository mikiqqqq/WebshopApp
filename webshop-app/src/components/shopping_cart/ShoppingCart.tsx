...
const ShoppingCart: React.FunctionComponent = () => {
    ...
    const emptyCart = async () => {
        ...
        try {
            ...
            await fetchOrderItems();  // Fetch order items again to ensure the cart is empty
        } catch (error) {
    ...
    useEffect(() => {
        const intervalId = setInterval(rotateIcon, 30000);
    ...
    const handleRemoveItem = useCallback(async (id: number) => {
        await fetchOrderItems(); // Re-fetch order items to update the list
    }, [fetchOrderItems]);
